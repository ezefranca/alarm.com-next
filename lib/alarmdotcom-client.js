...
class AlarmDotComClient {
  constructor({
    ...
    timeoutMs = 30 * 1000,
    userAgent = 'homebridge-alarmdotcom/0.2.0',
    logger = {}
  }) {
  ...
  _hasActiveSession() {
    return Boolean(this.ajaxKey && this.systemIds.length && Date.now() < this.sessionExpiresAt);
  }
}
...
function performHttpsRequest(method, url, headers, body, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    request.on('timeout', () => {
      request.destroy(new AlarmDotComError(`Alarm.com request timed out after ${timeoutMs}ms.`));
    });
