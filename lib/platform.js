...
const PLUGIN_NAME = 'homebridge-alarmdotcom-next';
const PLATFORM_NAME = 'AlarmdotcomNext';
const MANUFACTURER = 'Alarm.com';
const API_EVENT_DID_FINISH_LAUNCHING = 'didFinishLaunching';
...
class AlarmDotComPlatform {
  constructor(log, config, api) {
    ...
    this.api = api;
    this.Service = api && api.hap ? api.hap.Service : null;
    this.Characteristic = api && api.hap ? api.hap.Characteristic : null;
    ...
    this.api.on(API_EVENT_DID_FINISH_LAUNCHING, () => {
      void this.start();
    });
  }
  ...
  async start() {
    ...
    this.currentAuthToken = this.config.authToken || await this.tokenStore.getToken(this.config.username);
  ...
  handleStartupError(error, tokenPath) {
    if (error instanceof OtpRequiredError) {
      ...
      this.log.error(
        `Run "alarmdotcom-auth --username ${this.config.username} --token-file ${tokenPath}" and complete one of: ${methods}, import an existing token with "--auth-token", or paste a reusable token into the optional "authToken" config field.`
      );
      return;
    }
